...
const THRESHOLD_IMAGE = 0.08; // percentage of different pixels to consider the image different

let allImagesSimilar = 0;
  ...
  .forEach((file) => {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        const idx = (width * y + x) << 2; //each pixel is 4 bytes
